...
python cli.py index    → index a folder
python cli.py search   → search
python cli.py clear    → wipe the index
python cli.py status   → stats
"""
...
def cmd_help():
    ...
    lines = [
        ("python cli.py",           "interactive menu"),
        ("python cli.py index",     "index a folder (prompts for path)"),
        ("python cli.py index /p",  "index a specific folder"),
        ("python cli.py search",    "search (prompts for query)"),
        ('python cli.py search "q"',"search with inline query"),
        ("python cli.py clear",     "wipe the vector index"),
        ("python cli.py status",    "show stats and config"),
        ("python cli.py help",      "this screen"),
    ]
...
def main():
    ...
    if fn: fn()
        ...
        print(dim("  Run: python cli.py help\n"))
        sys.exit(1)
